...
def extract_audio(video_path, output_wav):
    try:
        ffmpeg.input(video_path).output(
            output_wav, ar=16000, ac=1, format="wav", acodec="pcm_s16le"
...
def transcribe_audio(audio_path, start_time, model_size="small", compute_type="int8"):
    ...
    with open(txt_filename, "w") as txt, open(srt_filename, "w") as srt:
        with tqdm(
            total=total_duration, unit="sec", desc="Transcribing", unit_scale=True
        ) as pbar:
...
def main():
    parser = argparse.ArgumentParser(
        description="Transcribe video using faster-whisper."
    )
    parser.add_argument("video", help="Path to video file")
    parser.add_argument(
        "--model",
        default="small",
        help="Model: tiny, base, small, medium, large-v1, large-v2, large-v3, large, distil-large-v2, distil-large-v3, large-v3-turbo, turbo",
    )
    parser.add_argument(
        "--compute",
        default="int8",
        help="Compute type: int8, int8_float32, float16, float32",
    )
